...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  ...
  const name = categorySlug.replaceAll("-", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
  return { title: `${name} news`, description: `The latest ${name} stories from News Platform.` };
}
