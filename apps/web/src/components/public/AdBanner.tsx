...
export function AdBanner({ advertisement, placement, category, className = "" }: { advertisement?: Advertisement; placement?: string; category?: string; className?: string }) {
  ...
  const content = <>
    ...
    <div className="mt-4 overflow-hidden rounded-2xl bg-mist">
      {loaded.mediaType === "VIDEO" ? <video src={loaded.mediaUrl} poster={loaded.thumbnailUrl ?? undefined} muted playsInline controls className="max-h-80 min-h-40 w-full object-cover" /> : <img src={loaded.mediaUrl} alt={`${loaded.advertiserName}: ${loaded.title}`} className="max-h-80 min-h-40 w-full object-cover" loading="lazy" />}
    </div>
    <div className="mt-4 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold text-ink">{loaded.advertiserName}</p><h2 className="mt-1 font-display text-2xl font-bold tracking-[-0.03em] text-ink">{loaded.title}</h2>{loaded.description ? <p className="mt-2 max-w-2xl text-sm leading-6 text-slate">{loaded.description}</p> : null}</div>{loaded.destinationUrl ? <span className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-ink px-4 text-sm font-bold text-white">Learn more <ExternalLink size={14} aria-hidden="true" /></span> : null}</div>
  </>;

  return loaded.destinationUrl ? <a href={loaded.destinationUrl} target="_blank" rel="noopener noreferrer" className={`block rounded-3xl border border-line bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lg sm:p-7 ${className}`}>{content}</a> : <section className={`rounded-3xl border border-line bg-white p-5 shadow-soft sm:p-7 ${className}`}>{content}</section>;
}
