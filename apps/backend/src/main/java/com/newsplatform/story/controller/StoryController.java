...
public class StoryController {
    ...
    @GetMapping("/feed") public FeedResponse feed(@RequestParam(required = false) String category, @RequestParam(required = false) String tag, @RequestParam(defaultValue = "20") int limit, @RequestParam(required = false) String cursor, @AuthenticationPrincipal AuthenticatedUser currentUser) { return service.publicFeed(category, tag, limit, cursor, currentUser == null ? null : currentUser.id()); }
    @GetMapping("/feed/{slug}") public StoryResponse publicStory(@PathVariable String slug, @AuthenticationPrincipal AuthenticatedUser currentUser) { return service.publicStory(slug, currentUser == null ? null : currentUser.id()); }
    @GetMapping("/admin/stories") @PreAuthorize("hasAuthority('STORY_VIEW_ADMIN') or hasAuthority('SUPER_ADMIN')") @SecurityRequirement(name = "bearerAuth") public List<AdminStoryResponse> list(@RequestParam(required = false) String search, @RequestParam(required = false) String status, @RequestParam(required = false) UUID categoryId, @RequestParam(required = false) Boolean breaking, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "50") int limit) { return service.listAdmin(search, status, categoryId, breaking, page, limit); }
